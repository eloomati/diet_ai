...
@router.get("", response_model=Page[TransactionResponse], status_code=status.HTTP_200_OK)
async def list_transactions(
    ...
    return Page(
        items=[TransactionResponse.from_result(result) for result in page.items],
        total=page.total,
    )
